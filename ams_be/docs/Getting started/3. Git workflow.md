...
- nhánh `main` hiện tại chỉ để lưu trữ và build code.
...
Dưới đây là một số type được định nghĩa sẵn:
- build: Các thay đổi ảnh hưởng đến hệ thống xây dựng hoặc các phụ thuộc bên ngoài.

- ci: Các thay đổi đối với các tệp cấu hình và tập lệnh CI của chúng tôi.
...
- refactor: Thay đổi mã không sửa lỗi cũng không thêm tính năng.

- test: Thêm các bài kiểm thử còn thiếu hoặc sửa các bài kiểm thử hiện có.
